...
const Select:FC<ISelect> = (props) => {
    ...
    return (
        <div onMouseLeave={() => setIsOpened(false)} ref={selectRef} className=" px-[10px] flex items-center hover:cursor-pointer relative ease-in duration-100 hover:bg-[#5b5b5b]"
        ...
        >
            ...
            <div  className={`${isOpened ? "h-260px w-[100%] py-2" : "h-[0px] py-[0px] overflow-hidden"} shadow-md absolute bg-white  left-0 top-[100%] z-10 text-center flex flex-col`}>
                {["All", ...props.list].map(item => (
                    ...
                    "bg-[#282828] text-white"}
                     rounded-[2px] text-[20px] font-bold mb-[10px] py-[5px] w-[90%] mx-auto last:my-0 ${selectedVariant !== item && 'hover:bg-[#6665]'}`}>
                        {item}
                    </div>
